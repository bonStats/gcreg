...
linear_dist_to_monotone_boundary <- function(gam, region, basis_cv, EPS = 1e-06){
  ...
  if(is.infinite(reg[1])) reg[1] <- -100
  if(is.infinite(reg[2])) reg[2] <- 100
  ...
  if(which_min_Dpl %in% c(1,2)) warning("linear distance to the monotone boundary based on constrained region = [", paste(reg, collapse = ","), "] instead of [", paste(region, collapse = ","), "]")
